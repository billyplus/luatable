...
`,
		...
		{ // table类型
			name:     "table测试",
			data:     [][]string{{"备注", "id", "名称", "类型", "头像", "攻击", "生命"}, {"", "sc", "sc", "sc", "c", "s", "s"}, {"comment", "int", "string", "int", "table", "int", "int"}, {"", "id", "name", "type", "icon", "attack", "life"}, {"1001", "1001", "名字1", "1", "{type=1,value=2}", "10", "100"}, {"1002", "1002", "名字2", "2", "{type=1,value=3}", "11", "101"}, {"1003", "1003", "", "3", "{type=1,value=4}", "12", "102"}, {"1004", "1004", "名字4", "4", "{type=1,value=5}", "13", "103"}},
			keyCount: 1,
			wants: `{
	[1001]={
		id=1001,
...
func TestOpenXLSXFile(t *testing.T) {
	...
	r := NewBaseReader("test", data[4:], "s", 0, 1, 2, 3, 4)
	result, err := r.ReadAll()
	assert.Nilf(err, "error reading content:%v", err)
	assert.Equalf(datawant, string(result), "%v 出错", "测试xlsx的base格式")
	if err = ioutil.WriteFile("./tmp.lua", result, 0644); err != nil {
		return
	}
}
